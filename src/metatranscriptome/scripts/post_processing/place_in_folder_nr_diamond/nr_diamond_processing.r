...
#alldiamondfiles <- fs::dir_ls(workingpath, glob="*nonzfhum.f.diamond.txt.gz", recurse = TRUE)
...
## revision for killifish - note the second remove that includes the nonzfhum. with extra dot removed
...
for(i in seq(alldiamondfiles)){
  #single_hit <- read_tsv(alldiamondfiles[i], col_names = FALSE)
  ...
  ## WILL WANT TO SEPARATELY PROCESS BLASTN THEN DIAMOND OUTPUTS, THEN IN STEP 1 FULL_JOIN
  
# Check first 1000 rows for NA or "N/A" values in the taxid column
  ...
  ## also removing artificial sequences, other, etc. at step A now - WE FIXED DIAMOND RESULTS BUT STILL NEED TO COMMENT OUT THE NEXT LINE
...
#  taxa <- getTaxonomy(taxid_unique,'//hpc/scratch/group.data.science/eric_temp/mmseqs_out/taxonomizr/nameNode.sqlite', desiredTaxa = c("superkingdom", "clade", "kingdom", "phylum", "class", "order", "family", "genus","species"))
  ## update 2025
#  taxa <- getTaxonomy(taxid_unique,'//hpc/scratch/group.data.science/eric_temp/databases/taxonomizr/feb2025taxonomy/nameNode.sqlite', desiredTaxa = c("superkingdom", "clade", "kingdom", "phylum", "class", "order", "family", "genus","species"))
    # update II
