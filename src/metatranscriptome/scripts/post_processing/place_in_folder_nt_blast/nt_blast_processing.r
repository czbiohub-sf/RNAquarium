...
for(i in seq(allblastnfiles)){
  ...
  #we should always have one of acellular root" or "domain" as NA so thus can re-create "superkingdom" column using one simple coalesce
  ## also rename
  ...
  if (length(missing_taxids) > 0) {
    #  Second pass with older (fallback) database
  ...
  ## ALSO ADDING FOR WHEN LOCAL BLAST DOES NOT HAVE TAXNAME - WE ARE PULLING SPECIES FROM TAXONOMIZR AFTER THE FACT
  ...
  ## UPDATED TAXONOMIZR CODE SO WE CAN SEARCH FOR SAR 
  ...
  ### CHANGING THIS AND ABOVE NA FROM LCA TO "Root_unresolved" since these are usually artificial, plasmids, unidentified, etc.
