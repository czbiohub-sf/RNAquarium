...
### if using a subdirectory /nt_blast
#workingpath0 <- getwd()
#workingpathdash0 <- str_c(workingpath0, "/")
#workingpath <- str_c(workingpath0, "/nt_blast")
#workingpathdash <- str_c(workingpath0, "/nt_blast/")
...
## for searching through all of Gibs' outputs use this version:
allblastnfiles <- fs::dir_ls(workingpath, glob="*nonzfhum.blast.txt.gz", recurse = FALSE)

## chunk names
chunknames <- allblastnfiles
chunknames <- str_replace(chunknames, workingpathdash, "")
chunknames <- str_replace(chunknames, "_nonzfhum.blast.txt.gz", "")

### the looped version - for a directory with many files, this will read one at a time


for(i in seq(allblastnfiles)){
#single_hit <- read_tsv(allblastnfiles[i], col_names = FALSE)
single_hit <- fread(allblastnfiles[i], verbose = TRUE, quote="")
# single_hit <- read_tsv(allblastnfiles, col_names = FALSE)

  # single_hit <- read_tsv(alldiamondfiles, col_names = FALSE)
...
# ## AUGUST 2025 - WHEN USING LOCAL BLASST WE DO NOT HAVE TAXNAME, SO NEED TO RE-CREATE USING SPECIES    
...
## sept 2025 - with updated code classifying N/A as root_unresolved, better not to remove these ahead of time!    
if (!all(top_hits$taxname[1:10] == "N/A")) {
  # Run full taxname + taxid cleaning
  #top_hits <- top_hits %>% filter(!grepl("N/A", taxname))
  #top_hits <- top_hits %>% filter(!grepl("vector", taxname, ignore.case = TRUE))
  #top_hits <- top_hits %>% filter(!grepl("synthetic", taxname, ignore.case = TRUE))
  # also remove 12908 32630 32644 - instead better NA filter below
  # Clean taxid and taxname
  top_hits$taxid <- gsub(";.*", "", top_hits$taxid)
  top_hits$taxname <- gsub("0.*", "", top_hits$taxname)
} else {
  # Only clean taxid if taxname is all "N/A" - correction can search for vector and synthetic in target_title
    ## removing synthetic in target_title is dangerous - would remove "photosynthetic" for example
  top_hits$taxid <- gsub(";.*", "", top_hits$taxid)
  #top_hits <- top_hits %>% filter(!grepl("vector", target_title, ignore.case = TRUE))
  #top_hits <- top_hits %>% filter(!grepl("synthetic", target_title, ignore.case = TRUE))
}
...
## also clade for SAR???
#taxa <- getTaxonomy(taxid_unique,'//hpc/scratch/group.data.science/eric_temp/mmseqs_out/taxonomizr/nameNode.sqlite', desiredTaxa = c("superkingdom", "clade", "kingdom", "phylum", "class", "order", "family", "genus","species"))

## update 2025 - TO AUG2025 TAXONOMY
#taxa <- getTaxonomy(taxid_unique,'//hpc/scratch/group.data.science/eric_temp/databases/taxonomizr/aug2025taxonomy/nameNode.sqlite', desiredTaxa = c("superkingdom", "clade", "kingdom", "phylum", "class", "order", "family", "genus","species"))
# update II
## AS PART OF UPDATED TAXONOMY - NEED TO COALESCE TWO FIELDS TO RECREATE OLD SUPERKINGDOM
 #we should always have one of acellular root" or "domain" as NA so thus I can re-create my old "superkingdom" column using one simple coalesce    
taxa <- getTaxonomy(taxid_unique,'//hpc/scratch/group.data.science/eric_temp/databases/taxonomizr/aug2025taxonomy/nameNode.sqlite', desiredTaxa = c("domain", "acellular root", "clade", "kingdom", "phylum", "class", "order", "family", "genus","species"))
...
if (length(missing_taxids) > 0) {
  ...
  taxa_second_pass <- getTaxonomy(missing_taxids, '//hpc/scratch/group.data.science/eric_temp/databases/taxonomizr/feb2025taxonomy/nameNode.sqlite', desiredTaxa = c("superkingdom", "clade", "kingdom", "phylum", "class", "order", "family", "genus", "species"))
...
## ALSO ADDING AUGUST 2025 FOR WHEN LOCAL BLAST DOES NOT HAVE TAXNAME - WE ARE PULLING SPECIES FROM TAXONOMIZR AFTER THE FACT

# Replace taxname with species only if the first 10 taxnames are all "N/A"
if (all(top_hitswithtaxa$taxname[1:10] == "N/A")) {
  # Replace taxname with species column
  top_hitswithtaxa$taxname <- top_hitswithtaxa$species
  
  # Replace NA (from missing taxonomy info) with "N/A"
  top_hitswithtaxa$taxname[is.na(top_hitswithtaxa$taxname)] <- "N/A"
}
...
### SEPT 2025 - CHANGING THIS AND BELOW NA FROM LCA TO "Root_unresolved" since these are usually artificial, plasmids, unidentified, etc.
## then replace_na
...
write.table(thresholded_hit_fishorprimates, file = paste0(currentchunkname,"_blastnclustered_hits_fishorprimates",".txt"), sep = "\t", row.names = FALSE, quote = FALSE, , col.names = FALSE)
...
thresholded_hit_nofishnomammals$taxoncategory2 <- ifelse((grepl("root", thresholded_hit_nofishnomammals$taxname_lca) == TRUE), "Root_unresolved",
    ...
    ### SEPT 2025 - CHANGING THIS AND ABOVE NA FROM LCA TO "Root_unresolved" since these are usually artificial, plasmids, unidentified, etc.
...
### AUGUST 2025 - FOR 75K RUN, WE HAVE 1 BIOPROJECT PRJXX - CONVERT TO PRJXX3472
## update - should skip
# thresholded_hit_nofishnomammals_blastn$query <- gsub("PRJXX", "PRJXX3472", thresholded_hit_nofishnomammals_blastn$query)
# thresholded_hit_nofishnomammals_blastn$bioproject <- gsub("PRJXX", "PRJXX3472", thresholded_hit_nofishnomammals_blastn$bioproject)
...
write.table(thresholded_hit_nofishnomammals_blastn, file = paste0(currentchunkname,"_blastnclustered_hits_nofishnohuman_",Sys.Date(),".tab"), sep = "\t", row.names = FALSE, quote = FALSE)

## now remove variables
rm(thresholded_hit_nofishnomammals)
rm(thresholded_hit_nofishnomammals_blastn)
