...
# Configuration
TSV_FILE="/hpc/projects/balla_group/sra_experiments/versioned_zf_output/75k_unstable/metatranscriptome_all/RNAquarium_outputs/taxonomy_hits_nonhost_mostrecent.tsv.gz"
NAMES_DMP="/hpc/scratch/group.data.science/eric_temp/databases/taxonomizr/aug2025taxonomy/names.dmp"
OUTPUT_FILE="/hpc/projects/balla_group/sra_experiments/versioned_zf_output/75k_unstable/metatranscriptome_all/RNAquarium_outputs/taxonomy_hits_nonhost_mostrecent_withtaxids.tsv.gz"
TEMP_DIR="temp_taxid_processing_nonhost"
...
## rename outputs
mv log_exact_taxnames.txt log_exact_taxnames_nonhost.txt
mv log_inexact_taxnames.txt log_inexact_taxnames_nonhost.txt
mv log_all_taxnames.txt log_all_taxnames_nonhost.txt
mv tsv_multiple_candidates.txt tsv_multiple_candidates_nonhost.txt
...
HEADER_COLS=$(zcat "$OUTPUT_FILE" | head -1 | awk -F'\t' '{print NF}')
if [ "$HEADER_COLS" -lt 53 ]; then
    echo "Error: File has only $HEADER_COLS columns, need at least 53 for tax_species_NTorNR"
    cd ..
    rm -rf "$TEMP_DIR_PART2"
    exit 1
fi
...
if [ "$NA_ROWS" -eq 0 ]; then
    ...
    # Step 2: Extract unique species names from column 53
    echo "Part II Step 2: Extracting unique species names from column 53..."
    tail -n +2 na_rows.tsv | cut -f53 | grep -v '^$' | grep -v '^N/A$' | sort -u > unique_species_names.txt
    ...
    if [ "$UNIQUE_SPECIES_COUNT" -eq 0 ]; then
        echo "No valid species names found in column 53 - copying original file"
        cp "$OUTPUT_FILE" "$PART2_OUTPUT_FILE"
        cd ..
        rm -rf "$TEMP_DIR_PART2"
    else
...
{
    # If this row has N/A in column 8 and valid species name in column 53
    if (NF >= 53 && $8 == "N/A" && $53 != "" && $53 != "N/A") {
        species_name = $53
        original_taxname = $7
...
EOF
        ...
        mv log_multiple_matches_and_assignments.txt log_multiple_matches_and_assignments_nonhost.txt
