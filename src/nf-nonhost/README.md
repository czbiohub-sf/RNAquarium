...
provide the following command line parameters:
 - `--ref_indexes <your reference genome indexes folder>`
 - `--ref_indexes_ercc <your reference genome + spike in indexes folder>`
 - `--gtf_no_ercc <reference_indexes.gtf>`

2. reference genome assembly and spike-in sequences (the required indexes will be generated for you)
provide the following command line parameters:
 - `--ref_genome <primary_assembly.fa>`
 - `--gtf_no_ercc <reference.gtf>`
 - `--ercc <ERCC.fa>`
 - `--ercc_gtf <ERCC.gtf>`

for example, for ensembl release 108 of the *Danio rerio* genome:
```
--ref_genome Danio_rerio.GRCz11.dna_sm.primary_assembly.fa \
--gtf_no_ercc Danio_rerio.GRCz11.108.gtf \
--ercc ERCC92.fa \
--ercc_gtf ERCC92.gtf
```
