...
process dedup {
	...
	input:
	tuple val(meta), path(mategz), path(star_log)
	// when would we ever /not/ have STAR logs and want to continue this readset?
	
	output:
	tuple val(meta), path("Unmapped.out.mate?.filteredbyBT.dedup.fastq.gz"), emit: mates
	tuple val(meta), stdout, emit: stats

	script:
	def PERC_COEFF = params.percentLen / 100
	def PRECONDITIONS="""if ! gzip -t ${mategz}; then
	...
	"""
	${PRECONDITIONS}
	gzip -cd ${mategz[0]} > Unmapped.out.mate1.filteredbyBT.fastq
	gzip -cd ${mategz[1]} > Unmapped.out.mate2.filteredbyBT.fastq
	...
	"""
	${PRECONDITIONS}
	gzip -cd ${mategz} > Unmapped.out.mate1.filteredbyBT.fastq
