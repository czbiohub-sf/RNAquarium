...
rm -rf ${WORKING_DIR}/logs/Bowtie2.PE.log
rm -rf ${WORKING_DIR}/logs/Bowtie2.SE.log
rm -rf ${WORKING_DIR}/logs/Bowtie2.fail.log

if [ ! -d "${WORKING_DIR}/Bowtie2_out" ]; then
    mkdir "${WORKING_DIR}/Bowtie2_out"
fi

sbatch step.4.Bowtie.sh "${WORKING_DIR}" "${ACCESSIONS_LIST}" "${TOOLS}" "${ENVNAME}" "${REMOVEFQ}"
