...
rm -rf ${WORKING_DIR}/logs/gsnap.process.log
rm -rf ${WORKING_DIR}/logs/gsnap.error.log


if [ ! -d "${WORKING_DIR}/Gsnap_out" ]; then
    mkdir "${WORKING_DIR}/Gsnap_out"
fi


sbatch step.6.gsnap.sh "${WORKING_DIR}" "${ACCESSIONS_LIST}" "${TOOLS}" "${ENVNAME}" "${REMOVEFQ}"
